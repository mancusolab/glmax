...
def test_standalone_wald_test_is_in_all():
    """Private wald helper is not exported from glmax._infer.infer.__all__."""
    import importlib

    infer_module = importlib.import_module("glmax._infer.infer")
    assert "wald_test" not in infer_module.__all__
    assert "_wald_test" not in infer_module.__all__
